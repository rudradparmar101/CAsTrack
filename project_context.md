...
> **Last updated:** 2026-07-10 (Phase 8 — type unification + deletions — complete)
...
> **Version control:** git, with a GitHub remote configured (`origin`). Working tree is clean; local commits are not yet pushed (pending explicit go-ahead — see Phase 8's ⚠ HUMAN item).
...
| Question | Answer |
|---|---|
| What phase are we in? | **Phase 8 complete** (type unification + deletions, zero behavior change). Dashboard (`admin-dashboard.tsx`/`member-dashboard.tsx`) now consumes `FirmTaskWithRefs` through a new shared `TaskSummaryCard`, replacing the deleted legacy `components/task-card.tsx` — there is now exactly **one** task type system (`FirmTask*`) across the whole app. Also removed: `markTaskCompleteAction`, orphaned `lib/activity.ts`/`lib/notifications.ts`, the `Organization`/`'admin'|'member'` legacy aliases, the deprecated `organization` field on `getAuthContext()`, and every dead legacy type in `lib/types.ts`. Legacy DeadlineTracker `supabase/` artifacts (old schema, migrations, cron, edge function) archived to `supabase/_legacy-deadlinetracker/`. Lint is now **fully clean** (was 2 errors + 4 warnings baseline through Phase 7) — the new baseline going forward. Phase 9 (CA-core schema extension) is next. |
| Does it build? | ✅ `npm run build` clean (incl. TypeScript). ✅ `npm run lint` — **zero errors, zero warnings** (Phase 8 fixed the last 2 pre-existing `notification-bell.tsx` errors + all 4 unused-var warnings). |
...
| What works? | Auth + 3 onboarding paths, Clients (CRUD/addresses/persons/portal invites), Documents (upload/versions/approve-reject, staff + portal), Tasks (list/detail/stage machine/assignment/comments/documents/activity), client portal (runtime-verified end-to-end), Team (departments + membership), Templates (department-scoped), Settings (firm rename), **Dashboard (partner/employee split, now on the unified FirmTask model)** — all working against the real schema and RLS-proven for the roles exercised. Full teal-accent light/dark theme across every page. |
| Type system | ✅ **Unified (Phase 8).** Every task-shaped surface in the app — `/tasks`, `/dashboard`, the portal — now reads `FirmTask*` from `lib/types.ts`. No parallel legacy type system remains. |
| Biggest risks right now | (1) RLS smoke testing (Ph7) covered the roles/paths in that phase's checklist, not an exhaustive policy-by-policy pass — Phase 14 is the dedicated final pass. (2) The dev-mode accept-invite→`/portal` redirect can race the client router (test-harness-observed, same class as the pre-existing `/onboarding` issue) — never reproduced against a production build, worth a real-build spot-check before shipping the portal. (3) Local commits are ahead of `origin/main` and not yet pushed — pending explicit go-ahead. |
| Verification gates | `npm run build` and `npm run lint` (now fully clean, zero-tolerance baseline), plus `scripts/verify/*.mjs` (admin API + Playwright, run manually with `node`) — all 6 scripts green from Phase 7 (92/93 individual assertions; the 1 failure is a documented redirect-timing flake, not a product bug), plus a Phase 8 Playwright spot-check of the rebuilt dashboard. Not wired into CI; still run by hand. |
...
| Dashboard (admin/member) | ✅ **Unified onto FirmTask (Ph8)** — same model as `/tasks`; `task-card.tsx` deleted in favor of a shared `TaskSummaryCard`. Retheme'd (Ph6); Playwright-verified for partner + employee (Ph8) |
