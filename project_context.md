...
> **Last updated:** 2026-07-19 (Phase 12.5 — Statutory identifiers: ARN capture + UDIN register — complete, bulk client import split to Phase 12.6; off-roadmap: dashboard card detail modals, branded forgot-password/reset-password flow, fee_masters rate-card management UI)
...
| Question | Answer |
|---|---|
| What phase are we in? | **Phase 12.5 complete** (statutory identifiers — ARN capture + UDIN register; bulk client import split out to Phase 12.6, not built). Needed a migration (007) — presented to Jay before building anything, applied 2026-07-19. `tasks.arn`/`tasks.filed_date` promoted out of `task_activities` (staff-only readable) onto real columns, written atomically with the stage-completion update; surfaced on the filing grid + staff task detail (deliberately NOT the portal this phase, Jay's explicit scope call). New `udin_register` table: capture-only firm register of ICAI UDINs, `reports.view`-gated reads, partner-only writes at the RLS layer (no new permission key — a flagged either/or Jay chose explicitly), staff-internal only with **zero** client_user access (RLS walked through role-by-role with Jay before the migration was applied). New `/udin` surface: list/create/edit, no delete UI. See §4.11. Phase 12 (client billing) itself completed the prior session — see §5. |
...
| What works? | Auth + 3 onboarding paths, Clients (CRUD/addresses/persons/**registrations**/portal invites), Documents (upload/versions/approve-reject, staff + portal), Tasks (list/detail/stage machine/assignment/comments/documents/activity/**filing outcomes, now with structured ARN/filed_date**), client portal (runtime-verified end-to-end), Team (departments + membership), Templates (department-scoped), Settings (firm rename), Dashboard (partner/employee split, unified FirmTask model), **Compliance (Ph10/12.5): registrations editor, calendar-driven statutory generation (manual + cron), filing-status grid (now shows ARN), UDIN register** — all working against the real schema and RLS-proven for the roles exercised. Billing (Ph12): invoices, receipts, outstanding ledger, rate-card management. Full teal-accent light/dark theme across every page. |
| Type system | ✅ **Unified (Phase 8)** and now **complete (Phase 10)**, extended Phase 12.5 — `lib/types.ts` carries every Phase 9 schema column (`FirmTask.source/category/period_*`, `Client.is_audit_applicable/audit_type`, `ClientRegistration`, `ComplianceType`) plus Phase 12.5's `FirmTask.arn/filed_date` and the new `UdinRegisterEntry`/`UdinRegisterEntryWithRefs`. |
...
### 4.10 Rate-card management (2026-07-19, off-roadmap — closes a Phase 12 gap, no schema change)

Phase 12 shipped `fee_masters` (schema + RLS, migrations 004+005) and the invoice create-modal's autofill read of it, but nothing in the app could create/edit/deactivate a rate — rows could only be seeded via direct DB access (§6's "fee_masters has no management UI" item, now resolved). This session built the missing write surface on the existing `fee_masters` table/RLS/`billing.view`/`billing.manage` permissions — no new migration.

- **Surface:** a third stacked "Rate Card" section on `/billing` (`billing-page-client.tsx`), between Outstanding Ledger and Invoices — matches the page's existing pattern of stacked sections rather than introducing a tab/sub-route. Lists every `fee_masters` row for the firm (active + inactive, unlike the invoice form's active-only read), joined to `client:client_id(name)` and `compliance_type:compliance_type_id(name)` for display (`FeeMasterWithRefs` in `lib/types.ts`).
- **Create/edit** (`fee-master-form.tsx`, modeled on `team-form.tsx`'s create/edit-via-one-form pattern): service name, amount, periodicity (`one_time`/`monthly`/`quarterly`/`annual`), an "Applies to" select (blank = firm-wide default rate, or a specific client — mirrors the schema's `client_id IS NULL` convention), an optional link to the `compliance_types` catalog, and free-text notes.
- **Deactivate/reactivate** (`toggleFeeMasterActiveAction`): `is_active` toggle only, same icon-button pattern as `team-page-client.tsx`'s department toggle — **no hard delete**, mirrors the clients/departments/compliance_types no-hard-delete precedent (the schema does have a DELETE RLS policy on `fee_masters` from Phase 12, but the app deliberately never calls it).
- **Guard + failure mode:** `requireBillingManage()` (pre-existing in `billing/actions.ts`, unchanged) gates all three write actions; every write uses `.insert()/.update().select('id').single()` and a new `rlsFriendly()` in `billing/actions.ts` (same PGRST116-to-friendly-message mapping as `tasks/actions.ts`'s, no shared import since the two route groups don't currently share a lib module for this) so an RLS-denied write fails loudly instead of silent-200. Reading is gated by `canView` (`billing.view`, computed once in `page.tsx`, already existed) — a `billing.view`-only user sees the list with no create/edit/deactivate controls, matching the page's existing `canManage`-gated "New Invoice" button.
- **Invoice-autofill interaction (verified, not changed):** `page.tsx`'s existing active-only `fee_masters` fetch (feeding `InvoiceForm`'s "Fill from rate card" dropdown) was left untouched; the new management list is a **separate, additional** all-rows fetch. Deactivating a row removes it from the invoice dropdown on the next render (via `revalidatePath('/billing')`, already wired into every action) without any change to `invoice-form.tsx` itself.
- **Verified 2026-07-19** end-to-end via Playwright against the live dev server + live Supabase project, as three distinct real accounts (partner-firm-owner, a `billing.manage` employee, a `billing.view`-only employee): create a rate → edit it → deactivate it → confirm it drops out of the invoice autofill dropdown → reactivate it → confirm it reappears; separately confirmed the `billing.view`-only account can read the rate card but has no create/edit/deactivate controls, AND that a raw authenticated PostgREST write from that account is independently rejected by RLS (not just hidden by the app-layer gate). `npm run build` + `npm run lint` both clean (zero errors/warnings, baseline unchanged).

### 4.11 Statutory identifiers — structured ARN + UDIN register (Phase 12.5, migration 007)

Phase 10 already captured ARN/filed-date at statutory-task completion, but only into `task_activities` — staff-only readable by RLS, so it could never reach the filing grid or (in principle) the client portal. This phase closed that gap and added a wholly new register the app had never touched before. **Needed a migration** (⚠ HUMAN gate, same as 001/002/004/005/006) — presented to Jay before building anything, applied 2026-07-19, confirmed clean.

- **`tasks.arn` / `tasks.filed_date`** (two new nullable columns, `supabase/ca-firm/migrations/007_udin_register_and_arn.sql`): promoted out of `task_activities` for the exact reason migration 002 promoted `checklist_items` — a plain column is covered by the *existing* tasks SELECT/UPDATE RLS (no new policy needed), an activity-log row is not. `changeStageCore()` (`tasks/actions.ts`) now writes both columns in the **same** `.update()` call as the stage transition to `completed`, then logs the pre-existing `filing_outcome_recorded` activity entry from the identical `filingOutcome` object — one atomic write, so the column and the audit-trail entry can never silently diverge (verified explicitly, not just "the grid renders").
- **Surfaced on:** the filing-status grid (`/compliance` — ARN shown as small text under the stage badge, only reachable once `isApplicable()` already let the cell resolve to a task) and the staff task detail page (`task-metadata.tsx`'s new "Filing outcome" block, ARN + formatted filed date). **Deliberately NOT surfaced on `/portal/tasks/[id]`** this phase — an explicit scope decision from Jay, not an RLS limitation (the columns are in fact covered by the task's normal `visible_to_client` gate, same as `checklist_items`; a future phase could add portal display with zero schema change).
- **UDIN register** (new `udin_register` table, migration 007): a firm-side **capture-only** record of a UDIN a signing CA member generated on ICAI's own portal — never a generator or validator against ICAI. Columns: `udin` (18-char alphanumeric CHECK, sub-structure deliberately not validated), `document_type` (free text, not an enum — the real-world taxonomy varies too much by firm/engagement to hardcode), `client_id`, `generated_on`, `signing_partner_id` (FK to `profiles`, not DB-restricted to `role='partner'` — an employee could legitimately be the actual signing member), optional `task_id`/`document_id` links (`ON DELETE SET NULL` — a UDIN record must outlive either), `notes`. No `is_active`/soft-delete column — a UDIN entry is a point-in-time historical fact, not something a firm "retires" (corrections go through edit, per the clients/departments/compliance_types/fee_masters no-hard-delete family, but with no toggle concept mapping to real usage here).
- **Permission gating — a flagged decision, not a unilateral one:** no existing permission key fit "manage a compliance register." Rather than inventing a new `compliance.manage` key, reads reuse the **existing** `reports.view` (identical gate to the filing grid); writes are **partner-only at the RLS layer itself** (`get_user_role() = 'partner'`, no permission-catalog key at all) — mirrors Phase 10's identical choice for statutory-task generation. Presented as an explicit either/or to Jay before the migration was applied; Jay chose the no-new-key version ("reversible later; correct for a single-firm pilot").
- **Client isolation — verified twice, not once:** before applying the migration, every `udin_register` RLS policy was walked through role-by-role with Jay from the raw SQL (not summarized) to confirm `client_user` has zero SELECT/INSERT/UPDATE/DELETE path — `has_permission('reports.view')` short-circuits to `false` for any role other than `partner`/`employee` *before* ever consulting `user_permissions`, and every write policy requires `get_user_role() = 'partner'`. No DEFINER RPC or curated view offers a side-door (unlike `client_invoices`/`client_invoice_items` in Phase 12, which exist specifically to give clients a curated read) — there is no `/portal` UDIN surface at all, and no plan to add one (a UDIN's audience is third-party verifiers via ICAI, not the client, and it's already printed on the signed document the client holds).
- **New surface:** `/udin` (partner-only sidebar link, `sidebar.tsx`) — list + create + edit, reusing the `fee-master-form.tsx`-style single-form-for-both pattern. No delete UI (schema allows it via RLS `DELETE`, app deliberately doesn't expose it — same precedent as `fee_masters`).
- **Verified 2026-07-19** end-to-end via Playwright against the live dev server + live Supabase project: completed a real statutory task with an ARN via the real UI, confirmed the `tasks.arn`/`tasks.filed_date` columns AND the `task_activities` entry agree (the explicit silent-divergence check); confirmed the grid and staff task detail both render it; created + edited a UDIN entry as a partner; confirmed a `reports.view`-only (non-partner) employee can read the register but has no create/edit controls in the UI, AND that a raw-PostgREST INSERT and UPDATE from that same account are both independently rejected by RLS. `npm run build` + `npm run lint` both clean (zero errors/warnings, baseline unchanged).
- **Not built this phase (tracked separately, see docs/ROADMAP.md Phase 12.6):** bulk client import from Excel/CSV — explicitly out of scope for this session (mixing it in would have violated one-phase-per-session); it needs no schema of its own yet, so it wasn't blocked on this migration.
...
| **12.5 — Statutory identifiers (partial: ARN + UDIN)** | 2026-07-19 | Migration 007 (⚠ HUMAN gate, presented before building anything, applied clean): `tasks.arn`/`tasks.filed_date` columns + new `udin_register` table + RLS. `changeStageCore()` now writes ARN/filed_date atomically with the completion stage change, alongside the pre-existing `task_activities` audit entry; surfaced on the filing grid + staff task detail (not the portal — explicit scope call). New `/udin` (partner-only nav, `reports.view`-gated reads, partner-only writes at the RLS layer — a flagged permission-key decision Jay resolved explicitly): list/create/edit, no delete. Bulk client import **not built** — split out to Phase 12.6 per the one-phase-per-session rule | ✅ Build + lint clean throughout; live Playwright pass verified the ARN column and the `task_activities` entry agree (explicit anti-divergence check), grid + task-detail rendering, UDIN create/edit, a `reports.view`-only account's read-but-no-write UI + two independent raw-PostgREST RLS rejections (INSERT and UPDATE) |
| 12.6+ | — | See docs/ROADMAP.md | ⏳ Not started |
...
| Client billing & receivables (invoices, receipts, outstanding ledger) | ✅ **Built (Ph12)** — `/billing` (staff) + `/portal/billing` (client, curated-view-only read path); fees-hold banner; **rate-card (`fee_masters`) management UI built 2026-07-19, off-roadmap** — see §4.10 |
...
| Compliance core (registrations, generation, filing grid) | ✅ **Built (Ph10)**, ARN capture made client/grid-visible (Ph12.5) — registrations editor, calendar-driven statutory generation (manual + cron), filing-status grid (now shows ARN under the stage badge), structured filing-outcome capture (`tasks.arn`/`tasks.filed_date`, migration 007). No period-selector/history view (current period only); no per-registration task granularity (§0 risk 4) |
| UDIN register | ✅ **Built (Ph12.5)** — `/udin` (staff-internal only, no `/portal` surface); list/create/edit, `reports.view`-gated reads, partner-only writes at the RLS layer (migration 007); no delete UI. See §4.11 |
