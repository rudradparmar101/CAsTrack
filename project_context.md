...
> **Last updated:** 2026-07-11 (Phase 10 — Compliance core build — complete)
...
| Question | Answer |
|---|---|
| What phase are we in? | **Phase 10 complete** (compliance core build, on top of Phase 9's schema — no new migration this phase). Delivered: (a) a statutory-registrations editor on the client form/detail page (repeatable sub-form, same JSON/replace-all pattern as addresses) + audit-applicability fields; (b) an idempotent calendar-driven generation engine (`lib/compliance/generation.ts` + `period.ts`) that upserts one statutory task per active-client × applicable-compliance-type × current-period, exposed via a partner-only "Generate now" action AND a Vercel-Cron-ready `/api/cron/generate-statutory-tasks` route (service-role, `CRON_SECRET`-gated); (c) a filing-status grid at `/compliance` (clients × compliance types, stage-colored cells, `reports.view`-gated); (d) filing-outcome capture (ARN/filed date) on statutory-task completion, logged to `task_activities` (no new tasks columns needed). Found + fixed a real bug in the process: the auth middleware was redirecting **every** unauthenticated request — including `/api/*` — to `/login`, which would have made the cron route completely unreachable in production; `/api/*` is now correctly treated as self-authenticating. Also found + fixed a real applicability-engine gap: `itr_non_audit_annual` had no negative condition, so it would have generated *alongside* `itr_audit_annual` for every audit-applicable client — now an explicit conflict-pair exclusion. Verified end-to-end via a new committed script (`scripts/verify/06-compliance-core.mjs`, 24/24 passing) that seeds a demo firm + 20 clients with mixed applicability and drives the real cron route twice (idempotency) plus a Playwright visual spot-check of the grid/client-detail/task-detail pages. Phase 11 (communication — Resend, reminders) is next. |
...
| Verification gates | `npm run build` and `npm run lint` (fully clean, zero-tolerance baseline), plus `scripts/verify/*.mjs` — now **7 scripts**: the Phase 7 suite (92/93, one documented flake) + Phase 10's `06-compliance-core.mjs` (24/24, admin-API + real cron-route calls, no Playwright needed for the core assertions) plus a throwaway (not committed) Playwright visual spot-check of `/compliance`, client detail, and task detail. Not wired into CI; still run by hand. |
...
| 11+ | — | See docs/ROADMAP.md | ⏳ Not started |
