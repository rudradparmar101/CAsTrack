...
**Deliberate exception — task-less documents are firm-wide, not department-scoped (accepted 2026-07-23, Phase 14.2 finding F5).** `can_access_document()`'s staff branch is `(task_id IS NOT NULL AND staff_can_access_task(task_id)) OR (task_id IS NULL AND (has_permission('clients.view') OR employee_has_task_for_client(client_id)))` — every task-linked document is department-scoped via `staff_can_access_task()`, but a **task-less** document (a client portal upload made before any task exists for it — e.g. a bank statement shared proactively) is visible to ANY employee holding `clients.view`, firm-wide, with no department check at all. This was empirically confirmed (`scripts/verify/14-rls-sweep.mjs`, Phase 14.1's sweep) and is **intentional, not a gap**: `clients.view` is the employee default specifically because most CA-firm employees are expected to see client-identifying data broadly, and a task-less document has no task to inherit a department scope from — `clients` themselves carry no department affiliation in this schema (a single client's tasks routinely span multiple departments: GST, income tax, audit, etc.), so "which department should a task-less upload belong to" has no natural answer without inventing a client-level department column that would need its own design decision (what does "a client's department" even mean when their tasks aren't all in one). Decided: accept firm-wide reach for task-less documents as correct, rather than force an artificial department binding. Revisit only if a real product need for department-scoped task-less documents emerges — at that point it's schema work (adding a department affiliation somewhere), not an RLS tweak.
