...
| `CRON_SECRET` | A real, randomly-generated production secret (the value in `.env.local` is a local-only placeholder — must be a fresh value in Vercel, not copied from `.env.local`) | Runtime | `/api/cron/generate-statutory-tasks` and `/api/cron/send-reminders` both reject with 401 if the bearer token doesn't match — statutory task generation and reminder emails simply never run. The `vercel.json` `crons` entries (added 2026-07-24 — `generate-statutory-tasks` daily at 03:00 UTC, `send-reminders` daily at 04:00 UTC) schedule both routes; Vercel automatically attaches the `Authorization: Bearer $CRON_SECRET` header to those invocations **once `CRON_SECRET` is set as a Vercel env var**. If `CRON_SECRET` is unset in Vercel, the scheduled calls arrive with no/no-matching bearer and the routes 401 — nothing runs. |
