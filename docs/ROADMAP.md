...
- **Public-endpoint rate limiting (2026-07-24, migration 019).** Closes project_context.md §6 item 9. DB-backed (Vercel is serverless — an in-memory limiter can't survive between invocations), one generic `rate_limit_buckets` table + atomic `check_rate_limit()` RPC, wired into signup (both modes), the join-firm invite-code lookup, forgot-password (email + IP, enumeration-safety preserved), and accept-invite lookup. `/login` deliberately excluded — it's a client-side call that never reaches this server; recorded as a decision with a revisit trigger, not a gap (`docs/DECISIONS.md`, 2026-07-24). `scripts/verify/15-rate-limiting.mjs` (19/19): atomic-concurrency proof (40 simultaneous callers, exact count), window-reset proof, fail-open proof, cron-cleanup proof, real-UI Playwright pass. Full 190-check `14-rls-sweep.mjs` re-run clean. Full detail: project_context.md §4.26 and `docs/DECISIONS.md`'s 2026-07-24 entries.
